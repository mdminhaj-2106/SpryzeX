...
func (e *Editor) View(focused bool) string {
        ...
        for row := e.ScrollRow; row < e.ScrollRow+e.height && row < len(e.Lines); row++ {
                ...
                // --- Gutter ---
                numStr := fmt.Sprintf("%*d", gutterW-2, lineNum)

                switch {
                case hasDiag:
                        indicator := lipgloss.NewStyle().
                                Foreground(theme.ColorError).Background(theme.BgSurface).
                                Bold(true).Render("▎")
                        numPart := lipgloss.NewStyle().
                                Foreground(theme.ColorError).Background(theme.BgSurface).
                                Bold(true).Render(numStr + " ")
                        sb.WriteString(indicator + numPart)

                case isCursorRow && focused:
                        indicator := lipgloss.NewStyle().
                                Foreground(theme.SpryzexGlow).Background(theme.BgSurface).
                                Bold(true).Render("▎")
                        numPart := lipgloss.NewStyle().
                                Foreground(theme.SpryzexBright).Background(theme.BgSurface).
                                Bold(true).Render(numStr + " ")
                        sb.WriteString(indicator + numPart)

                default:
                        indicator := lipgloss.NewStyle().
                                Foreground(theme.BgOverlay).Background(theme.BgSurface).
                                Render("▎")
                        numPart := lipgloss.NewStyle().
                                Foreground(theme.TextMuted).Background(theme.BgSurface).
                                Render(numStr + " ")
                        sb.WriteString(indicator + numPart)
                }
                ...
                if hasDiag {
                        sepChar = "│"
                        sepStyle = lipgloss.NewStyle().Foreground(theme.ColorError).Background(theme.BgSurface).Bold(true)
                } else if focused && isCursorRow {
                        sepChar = "│"
                        sepStyle = lipgloss.NewStyle().Foreground(theme.SpryzexRed).Background(theme.BgSurface)
                } else {
                        sepChar = "│"
                        sepStyle = lipgloss.NewStyle().Foreground(theme.BorderActive).Background(theme.BgSurface)
                }
                sb.WriteString(sepStyle.Render(sepChar))

                // --- Line content ---
                rendered := e.renderLine(e.Lines[row], contentW, isCursorRow && focused)
                if isCursorRow && focused {
                        bg := lipgloss.NewStyle().Background(theme.BgSelect).Width(contentW)
        ...
        body := sb.String()
        renderedLines := strings.Count(body, "\n")
        for i := renderedLines; i < e.height; i++ {
                emptyGutter := lipgloss.NewStyle().
                        Foreground(theme.BgOverlay).Background(theme.BgSurface).Render("▎") +
                        lipgloss.NewStyle().
                                Foreground(theme.BgMuted).Background(theme.BgSurface).
                                Render(strings.Repeat(" ", gutterW-2) + " ")
                sepEmpty := lipgloss.NewStyle().Foreground(theme.BorderActive).Background(theme.BgSurface).Render("│")
                tilde := lipgloss.NewStyle().Foreground(theme.BgOverlay).Background(theme.BgSurface).Render("~")
                padding := lipgloss.NewStyle().Background(theme.BgSurface).Render(strings.Repeat(" ", max(contentW-1, 0)))
                sb.WriteString(emptyGutter + sepEmpty + tilde + padding + "\n")
        }
